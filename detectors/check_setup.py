...
def check_setup():
    ...
    if os.path.exists('venv'):
        ...
        if in_venv:
            print(f"   [OK] Currently activated: {sys.prefix}")
        else:
            print(f"   [WARN]  Not activated. Run: source venv/bin/activate")
    else:
    ...
    if os.path.exists('RT-DETR'):
        print("   [OK] RT-DETR repository cloned")
        
        # Check specific directories
        if os.path.exists('RT-DETR/rtdetr_pytorch'):
            print("   [OK] PyTorch implementation found")
        else:
            print("   [ERROR] RT-DETR/rtdetr_pytorch not found")
            all_good = False
        ...
        if os.path.exists('RT-DETR/rtdetr_pytorch/checkpoints/rtdetr_r50vd_coco.pth'):
            ...
            print("   [WARN]  Pretrained model not found")
            print("      This will be downloaded when you run setup_rtdetr.py")
    else:
        print("   [ERROR] RT-DETR repository not cloned")
    ...
    if image_files:
        print(f"   [OK] Found {len(image_files)} image(s):")
        for img in image_files[:5]:  # Show first 5
            print(f"      - {img}")
        if len(image_files) > 5:
            print(f"      ... and {len(image_files) - 5} more")
    else:
        print("   [WARN]  No image files found in current directory")
    ...
    if all_good:
        ...
        print("[ERROR] Some issues found. Please fix them first.")
